...
## 本ドキュメントに関する注意事項[​](#本ドキュメントに関する注意事項 "本ドキュメントに関する注意事項 への直接リンク")

本ドキュメントははてなブログにおける Atom Publishing Protocol の仕様を解説するものです。

## Atom Publishing Protocol とは[​](#atom-publishing-protocol-とは "Atom Publishing Protocol とは への直接リンク")
...
AtomPub について詳しくは [http://www.ietf.org/rfc/rfc5023.txt](http://www.ietf.org/rfc/rfc5023.txt) (英語)などを参照してください。

## URIの表記について[​](#uriの表記について "URIの表記について への直接リンク")
...
有料プラン（はてなブログPro、はてなブログBusinessなど）の独自ドメイン機能をご利用の方は、独自ドメイン設定前のブログのドメインがブログのIDとなります。ブログの[詳細設定](https://blog.hatena.ne.jp/my/config/detail)のAtomPub項内のルートエンドポイントをご参照ください。

## はてなブログにおける AtomPub 実装の概要[​](#はてなブログにおける-atompub-実装の概要 "はてなブログにおける AtomPub 実装の概要 への直接リンク")
...
OAuth認証の詳細に関しては、[はてなのOAuthを利用する方法](https://developer.hatena.ne.jp/ja/documents/auth/apis/oauth/)を参照してください。はてなブログAtomPub では、全操作に関して [read\_private](https://developer.hatena.ne.jp/ja/documents/auth/apis/oauth/scopes#read_private) 操作 及び [write\_private](https://developer.hatena.ne.jp/ja/documents/auth/apis/oauth/scopes#write_private) 操作の承認を得ている必要があります。

WSSE認証の詳細に関しては[はてなサービスにおける WSSE認証](https://developer.hatena.ne.jp/ja/documents/auth/apis/wsse)を御覧ください。

Basic認証についてはユーザ名としてはてなIDを、パスワードとしてAPIキーを利用することで認証できます。Basic認証はAPIのURLがhttpsではじまる場合にのみ利用できます。

本WSSE認証における、ユーザ名にははてなIDを、パスワードには、ブログの[詳細設定](https://blog.hatena.ne.jp/my/config/detail)に記載されたAPIキーをご利用下さい。

## 文字コード[​](#文字コード "文字コード への直接リンク")

はてなブログ AtomPub では文字コードとして UTF-8 を利用します。リクエストXML 、レスポンスXML 共に UTF-8 として扱ってください。

## サービス文書[​](#service "サービス文書 への直接リンク")

はてなブログ AtomPub で操作できるコレクションの一覧を含むサービス文書を取得できます。

固定ページに関するコレクションの一覧は、固定ページを利用可能なブログでのみ表示されます。

### リクエスト[​](#リクエスト "リクエスト への直接リンク")

```
GET https://blog.hatena.ne.jp/{はてなID}/{ブログID}/atom
```

### レスポンス[​](#レスポンス "レスポンス への直接リンク")
...
## コレクション[​](#コレクション "コレクション への直接リンク")

はてなブログのエントリ・固定ページを操作するためのコレクションです。ブログエントリ・固定ページの一覧取得、新規投稿を行うことができます。

### ブログエントリの一覧取得[​](#ブログエントリの一覧取得 "ブログエントリの一覧取得 への直接リンク")
...
### ブログエントリの投稿[​](#ブログエントリの投稿 "ブログエントリの投稿 への直接リンク")
...
### 固定ページの一覧取得[​](#固定ページの一覧取得 "固定ページの一覧取得 への直接リンク")
...
### 固定ページの投稿[​](#固定ページの投稿 "固定ページの投稿 への直接リンク")
...
### ブログエントリの編集[​](#ブログエントリの編集 "ブログエントリの編集 への直接リンク")
...
### ブログエントリの削除[​](#ブログエントリの削除 "ブログエントリの削除 への直接リンク")
...
## アクセス可能なユーザ[​](#アクセス可能なユーザ "アクセス可能なユーザ への直接リンク")
...
## エラーレスポンス[​](#エラーレスポンス "エラーレスポンス への直接リンク")

はてなブログ AtomPub に対して不正な操作を行った場合にエラーレスポンスが返却されます。各エラーレスポンスには次のような意味があります。

#### 400 Bad Request[​](#400-bad-request "400 Bad Request への直接リンク")

リクエストが不正な場合に返却されます。リクエストに含まれるXMLの内容等を確認してください。

WSSEによる認証が失敗した際に返却されます。

#### 404 Not Found[​](#404-not-found "404 Not Found への直接リンク")

存在しないリソースにアクセスした際に返却されます。URLに含まれるはてなID、dateおよびentry\_id等を確認してください。

#### 405 Method Not Allowd[​](#405-method-not-allowd "405 Method Not Allowd への直接リンク")

リソースに対して許可されていないメソッドによるリクエストを発行した際に返却されます。指定のリソースに対して可能な操作を確認してください。

#### 500 Internal Server Error[​](#500-internal-server-error "500 Internal Server Error への直接リンク")

はてなブログAtomPub 側でなんらかの問題が発生した際に返却されます。

## はてなブログ AtomPub を利用したプログラムの例[​](#はてなブログ-atompub-を利用したプログラムの例 "はてなブログ AtomPub を利用したプログラムの例 への直接リンク")

### Perlを用いた例 (WSSE認証)[​](#perlを用いた例-wsse認証 "Perlを用いた例 (WSSE認証) への直接リンク")
...
### Perl を用いた例 (OAuth認証)[​](#perl-を用いた例-oauth認証 "Perl を用いた例 (OAuth認証) への直接リンク")
...
`<YOUR CONSUMER KEY>`, `<YOUR CONSUMER SECRET>` には、自分のOAuthアプリケーションの consumer key, consumer secretを設定してください。`<YOUR BLOG API ROOT ENDPOINT>` には、[サービス文書のURI](#service)を設定してください。特に、はてなブログ AtomPub APIは、はてなのOAuth認証における、read\_private, write\_privateの両方を要求することに注意してください。詳しくは、[認証についての章](#auth)を参照してください。

スクリプトが起動したのち、[http://localhost:3000](http://localhost:3000/)へアクセスしてください。
...
### Ruby を用いた例[​](#ruby-を用いた例 "Ruby を用いた例 への直接リンク")
...
### Scala を用いた例[​](#scala-を用いた例 "Scala を用いた例 への直接リンク")
...
*   [RFC5023 Atom Publishing Protocol](https://www.ietf.org/rfc/rfc5023.txt)
*   [LWP::Authen::Wsse](https://search.cpan.org/perldoc?LWP::Authen::Wsse)
*   [XML::Atom](https://search.cpan.org/perldoc?XML::Atom)
*   [XML::Atom::Service](https://search.cpan.org/perldoc?XML::Atom::Service)
*   [Apache Abdera](https://abdera.apache.org/)
*   [Apache Commons Codec](https://commons.apache.org/proper/commons-codec/)
*   [はてなサービスにおけるOAuth](https://developer.hatena.ne.jp/ja/documents/auth/apis/oauth/)
*   [はてなサービスにおけるWSSE認証](https://developer.hatena.ne.jp/ja/documents/auth/apis/wsse)
*   [はてなXML名前空間](https://developer.hatena.ne.jp/ja/documents/other/misc/xmlns)

## 変更履歴[​](#変更履歴 "変更履歴 への直接リンク")
